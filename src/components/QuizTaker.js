...
export default function QuizTaker() {
  const { state } = useLocation(); // { docId, type, questions, text }
  const { currentUser } = useAuth();
  const navigate = useNavigate();

  const [questions, setQuestions] = useState([]);
  const [answers, setAnswers] = useState({});
  const [documentText, setDocumentText] = useState(state.text || "");
  ...
  useEffect(() => {
    async function fetch() {
      ...
      if (!state.text && state.docId && currentUser) {
        const snap = await getDocs(collection(db, "users", currentUser.uid, "documents"));
        const docData = snap.docs.find(doc => doc.id === state.docId);
        if (docData) setDocumentText(docData.data().text || "");
      }
    }
