...
// ✅ Async hash helper using Web Crypto API
...
export async function shareToMarketplace(userId, document, tags = []) {
    ...
    const textHash = await computeTextHash(document.text); // ✅ await
...
export async function importMarketplaceDoc(currentUserId, sharedDocId) {
    ...
    const sharedTextHash = sharedData.textHash || await computeTextHash(sharedData.text); // ✅ await
